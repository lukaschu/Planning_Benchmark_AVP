# Add the TOPP folder to the Python path
...
def TrajString5thDegree(q_beg, q_end, qs_beg, qs_end, qss_beg, qss_end, duration):
    ...
    trajectorystring += "%f\n%d"%(duration, ndof)
    for k in range(ndof):
        a, b, c, d, e, f = Interpolate5thdegree(q_beg[k], q_end[k], qs_beg[k], 
...
def CheckDOFLimits(trajectorystring, constraintsstring0):
    trajinfo = trajectorystring.split("\n")
    dur = float(trajinfo[0])
    ndof = int(trajinfo[1])
    
    for i in range(ndof):
        coeff_list = [float(j) for j in trajinfo[i + 2].split()]
        ...
        if (len(criticalqlist) == 0):
            ...
            #    if (not (abs(q(j)) <= float(constraintsstring0.split()[i+4]))): # This is slightly strange (L.S) 
            #        return False
